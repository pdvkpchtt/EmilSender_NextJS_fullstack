...
const MailReset = ({ url = "" }) => {
  return (
    <Tailwind>
      <Head>
        <title>Сброс пароля в EmailSender</title>
      </Head>
      <Body className="w-full bg-transparent flex flex-col items-center justify-center rounded">
        <Body className="w-[560px] bg-[#f6f6f8] pb-[32px] text-center rounded-[20px] mx-auto">
          <Text
            className={`text-[#00afff] text-[32px] font-bold leading-[38.4px] tracking-[-0.023em] p-0`}
          ...
          <Button
            href={url}
            className={`text-[#fff] text-[14px] cursor-pointer font-medium leading-[16px] tracking-[-0.21px] px-[16px] py-[12px] rounded-[16px] bg-[#00afff]   transition-all duration-[250ms]`}
          >
            Сбросить пароль
          </Button>
